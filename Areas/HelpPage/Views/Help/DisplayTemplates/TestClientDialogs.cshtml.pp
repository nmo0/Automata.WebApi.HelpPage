...
<div id="testClientDialog" title="@description.HttpMethod @description.RelativePath">
  ...
  <div data-bind="visible: ShouldShowBody">
    ...
    <pre><textarea class="sampleArea" spellcheck="false" data-bind="value: RequestBody, valueUpdate: 'afterkeydown'" rows="15"></textarea></pre>
  </div>
</div>
...
<script>
  testClientModel = {
    ...
    Samples: {
      @Html.Raw(@String.Join(",", Model.SampleRequests.Select(s => String.Format("\"{0}\": \"{1}\"", s.Key, HttpUtility.UrlEncode(s.Value.ToString(), System.Text.Encoding.UTF8))).ToArray()))
    },
